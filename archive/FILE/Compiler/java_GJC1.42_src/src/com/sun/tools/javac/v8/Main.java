...
import com.sun.tools.javac.v8.code.Source;
...
public class Main {
	...
	private class Option {
		...
		/**
		 * Print a line of documentation describing this standard or extended
		 * options.
		 */
		void xhelp() {
			help();
	...
	private Option[] recognizedOptions = {
			...
			new Option("-source", "opt.arg.release", "opt.source") {
				...
				boolean process(String option, String operand) {
					...
					boolean err = super.process(option, operand);
					String targetString = (String) options.get("-target");
					Target target = (targetString != null) ? Target
							.lookup(targetString) : null;
					if (source.ordinal >= Source.JDK1_4.ordinal) {
						if (target != null) {
			...
			new Option("-target", "opt.arg.release", "opt.target") {
				...
				boolean process(String option, String operand) {
					...
					Source source = (sourceString != null) ? Source
							.lookup(sourceString) : null;
					if (source != null && target != null
							&& source.ordinal >= Source.JDK1_4.ordinal
							&& target.ordinal < Target.JDK1_4.ordinal) {
						error("err.source.target.conflict");
						return true;
					}
					return err;
			...
			new XOption("-Xbootclasspath/p:", "opt.arg.path",
				...
				String helpSynopsis() {
					return name + getLocalizedString(argsNameKey);
				}
			...
			new XOption("-Xstdout", "opt.arg.file", "opt.Xstdout") {
				...
				boolean process(String option, String arg) {
					try {
						...
						error("err.error.writing.file", arg, e.toString());
						return true;
					}
			...
			}, new HiddenOption("-O"), new HiddenOption("-Xjcov"),
			new HiddenOption("-XD") {
	...
	void error(String key) {
		Log.printLines(out, ownName + ": "
				+ getLocalizedString(key, null, null));
		help();
	}
	...
	public int compile(String[] args) {
		Context context = new Context();// 编译上下文
		options = Options.instance(context);
		filenames = new ListBuffer();
		JavaCompiler comp = null;
		try {// 如果args参数是0跳出usage帮助
			if (args.length == 0) {
				help();
				return EXIT_CMDERR;
			}
			processArgs(forcedOpts);//处理参数
			List filenames;
			try {
				filenames = processArgs(CommandLine.parse(args));
				if (filenames == null) {// 源文件List,有多个源文件参与编译
					return EXIT_CMDERR;
				...
				Log.printLines(out, ownName
						+ ": "
						+ getLocalizedString("err.file.not.found", e
								.getMessage()));
				return EXIT_SYSERR;
			}
			context.put(Log.outKey, out);// 上下文开始压入log
			comp = JavaCompiler.make(context);//make开始make
			if (comp == null)
				return EXIT_SYSERR;
			List cs = comp.compile(filenames);//开始编译............进入
			if (comp.errorCount() != 0)
				return EXIT_ERROR;//显示编译错误
	...
	void bugMessage(Throwable ex) {
		Log.printLines(out, getLocalizedString("msg.bug", JavaCompiler
				.version()));

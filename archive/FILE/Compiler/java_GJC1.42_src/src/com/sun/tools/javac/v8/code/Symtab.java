...
public class Symtab implements Flags, ByteCodes {
	...
	public static Symtab instance(Context context) {
		...
		if (instance == null)
			instance = new Symtab(context);// 创建的'词法表'
		return instance;
	}
	...
	public void initType(Type type, String name) {
		initType(type, new ClassSymbol(PUBLIC, names.fromString(name), type,
				rootPackage));
	}
	...
	private VarSymbol enterConstant(String name, Type type) {
		VarSymbol c = new VarSymbol(PUBLIC | STATIC | FINAL, names
				.fromString(name), type, predefClass);
	 ...
	 * @param opcode
	 *            The operation's bytecode instruction.
	 */
	private void enterBinop(String name, Type left, Type right, Type res,
			int opcode) {
		predefClass.members().enter(
				new OperatorSymbol(names.fromString(name), new MethodType(List
						.make(left, right), res, Type.emptyList, methodClass),
						opcode, predefClass));
	}
	...
	private void enterBinop(String name, Type left, Type right, Type res,
		...
		enterBinop(name, left, right, res, (opcode1 << ByteCodes.preShift)
				| opcode2);
	}
	...
	private OperatorSymbol enterUnop(String name, Type arg, Type res, int opcode) {
		OperatorSymbol sym = new OperatorSymbol(
	...
	private Symtab(Context context) throws CompletionFailure {
		...
		errSymbol = new ClassSymbol(PUBLIC | STATIC, names.any, null,
				rootPackage);
		...
		arrayClassType.interfaces_field = List.make(cloneableType,
				serializableType);
		arrayClass.members_field = new Scope(arrayClass);
		lengthVar = new VarSymbol(PUBLIC | FINAL, names.length, intType,
				arrayClass);
		...
		falseConst = enterConstant("false", booleanType
				.constType(new Integer(0)));

...
/**
 * 这个词法分析器,映射输入的ASCII流和UNICODE转义字符为->token符号顺序 Tokens接口中全是常量符号. The lexical
 * analyzer maps an input stream consisting of ASCII characters and Unicode
 * escapes into a token sequence.
 * 
 */
public class Scanner implements Tokens, LayoutCharacters {
	...
	/**
	 * The input buffer, index of next chacter to be read, index of one past
	...
	/**
	 * 扫描器,构造器 Construct a scanner from a file input stream, a log, and a
	 * character encoding.
	 */
	 ...
	 * 
	 * @param radix
	 *            The radix of the number; one of 8, 10, 16.
	 */
	...
	/**
	 * Read longest possible sequence of special characters and convert to
	 * token.
	 */
	...
	/**
	 * Scan a doccomment line after the inital '*''s for a @deprecated tag. This
	 * should be extended to support all javadoc tags.
	 */
	...
	/**
	 * Skip a non-documentation comment. This method should be called once the
	 * initial /, * and the next character have been read.
	 */
	...
	/**
	 * 
	 * Scan a documention comment and return it as a string. This method should
	 * be called once the initial /, * and * have been read. It gathers the
	 * content of the comment (witout leading spaces and '*'s) in the string
	 * buffer. Stops on the close '/'.
	 */
	private String scanDocComment() {
		...
		outerLoop: while (bp < buflen) {
			wsLoop: while (bp < buflen) {
				switch (ch) {
				case ' ':
					scanDocCommentChar();
					break;

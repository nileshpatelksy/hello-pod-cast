 ...
 * subclass==就是内类的文法
 * 变量,方法,操作
 * Root class for Java symbols. It contains subclasses for specific sorts of
 * symbols, such as variables, methods and operators, types, packages. Each
 * subclass is represented as a static inner class inside Symbol.
 */
public class Symbol implements Flags, Kinds, TypeTags {
	...
	/**
	 * An accessor method for the flags of this symbol. Flags of class symbols
	 * should be accessed through the accessor method to make sure that the
	 * class symbol is loaded.
	 */
	public long flags() {
		return flags_field;
	}
	 ...
	 * reporting. Use of this method may result in the loss of the symbol's
	 * description.
	 */
	...
	/**
	 * The external type of a symbol. This is the symbol's erased type except
	 * for constructors of inner classes which get the enclosing instance class
	...
	public Type externalType() {
		...
		if (name == name.table.init && owner.hasOuterInstance()) {
			...
			return new MethodType(t.argtypes().prepend(outerThisType), t
					.restype(), t.thrown(), t.tsym);
		} else {
			return t;
		}
	}
	...
	/**
	 * Is this symbol declared (directly or indirectly) local to a method or
	 * variable initializer? Also includes fields of inner classes which are in
	...
	/**
	 * The fully qualified name of this symbol. This is the same as the symbol's
	 * name except for class symbols, which are handled separately.
	 */
	 ...
	 * representation. This is the same as the symbol's name except for class
	 * symbols, which are handled separately.
	 */
	...
	/**
	 * An inner class has an outer instance if it is not an interface it has an
	 * enclosing instance class which might be referenced from the class. Nested
	 * classes can see instance members of their enclosing class. Their
	 * constructors carry an additional this$n parameter, inserted implicitly by
	 * the compiler.
	 * 
	 * @see isInner
	 */
	...
	/**
	 * Fully check membership: hierarchy, protection, and hiding. Does not
	 * exclude methods not inherited due to overriding.
	 */
	public boolean isMemberOf(TypeSymbol clazz) {
		return owner == clazz || clazz.isSubClass(owner)
				&& isInheritedIn(clazz)
	...
	/**
	 * Check for hiding. Note that this doesn't handle multiple (interface)
	 * inheritance.
	 */
	...
	/**
	 * Is this symbol inherited into a given class? PRE: If symbol's owner is a
	 * interface, it is already assumed that the interface is a superinterface
	 * of given class.
	 * 
	 * @param clazz
	 *            The class for which we want to establish membership. This must
	 *            be a subclass of the member's owner.
	 */
	public boolean isInheritedIn(Symbol clazz) {
		...
		case 0:
			...
			for (Symbol sup = clazz; sup != null && sup != this.owner; sup = sup.type
					.supertype().tsym)
	...
	/**
	 * The (variable or method) symbol seen as a member of given class
	 * type`site' (this might change the symbol's type).
	 */
	...
	public static class TypeSymbol extends Symbol {
		...
		/**
		 * The rank of a class is the length of the longest path between the
		 * class and java.lang.Object in the class inheritance graph. Undefined
		 * for all other type symbols.
		 */
		public int rank() {
			throw new AssertionError();
		}
		...
		/**
		 * If symbol is a package, does it exist? Otherwise always true.
		 */
	...
	public static class PackageSymbol extends TypeSymbol {
		...
		/**
		 * The Java source which this symbol represents. Use of this method will
		 * result in the loss of the plain-language description for the symbol.
		 */
		public String toJava() {
		...
		/**
		 * A package "exists" if a type or package that exists has been seen
		 * within it.
		 */
		public boolean exists() {
	...
	public static class ClassSymbol extends TypeSymbol {
		...
		/**
		 * the fully qualified name of the class, i.e. pck.outer.inner. null for
		 * anonymous classes
		 */
		public Name fullname;
		 ...
		 * representation, i.e. pck.outer$inner, set externally for local and
		 * anonymous classes
		 */
		public Name flatname;
		...
		/**
		 * the classfile from where to load this class this will have extension
		 * .class or .java
		 */
		...
		public String className() {
			if (name.len == 0)
				return Log.getLocalizedString("anonymous.class", flatname
						.toString());
			else
				return fullname.toString();
		}
		...
		/**
		 * The rank of a class is the length of the longest path to Object in
	...
	public static class VarSymbol extends Symbol {
		...
		public Symbol asMemberOf(Type site) {
			return new VarSymbol(flags_field, name, site.memberType(this),
					owner);
		}
	}
	...
	public static class MethodSymbol extends Symbol {
		 ...
		 * 
		 * @param c
		 *            The class whose members are searched for implementations
		 */
		public Symbol implemented(TypeSymbol c) {
			Symbol impl = null;
			for (List is = c.type.interfaces(); impl == null && is.nonEmpty(); is = is.tail) {
				TypeSymbol i = ((Type) is.head).tsym;
				for (Scope.Entry e = i.members().lookup(name); impl == null
		...
		/**
		 * Will the erasure of this method be considered by the VM to override
		 * the erasure of the other when seen from class `origin'?
		 */
		...
		/**
		 * The implementation of this (abstract) symbol in class origin, from
		 * the VM's point of view, null if method does not have an
		 * implementation in class.
		 * 
		 * @param origin
		 *            The class of which the implementation is a member.
		 */
		public MethodSymbol binaryImplementation(ClassSymbol origin) {
			for (TypeSymbol c = origin; c != null; c = c.type.supertype().tsym) {
				for (Scope.Entry e = c.members().lookup(name); e.scope != null; e = e
						.next()) {
					if (e.sym.kind == MTH
							&& ((MethodSymbol) e.sym).binaryOverrides(this,
									origin))
		 ...
		 * members of class `origin'? It is assumed that both symbols have the
		 * same name. See JLS 8.4.6.1 (without transitivity) and 8.4.6.4
		 */
		...
		/**
		 * The implementation of this (abstract) symbol in class origin; null if
		 * none exists. Synthetic methods are not considered as possible
		 * implementations.
		 */
		...
		public Symbol asMemberOf(Type site) {
			return new MethodSymbol(flags_field, name, site.memberType(this),
					owner);
		}
	}
